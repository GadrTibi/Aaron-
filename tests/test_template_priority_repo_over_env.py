...
def test_repo_templates_have_priority_over_env(monkeypatch, tmp_path, kind, ext):
    ...
    base_repo_dir = repo_root / "templates" / kind
    repo_dir = base_repo_dir
    if kind == "estimation":
        repo_dir = base_repo_dir / "cd"
    ...
    monkeypatch.setattr(template_roots, "ESTIMATION_TPL_DIR", base_repo_dir)
    monkeypatch.setattr(template_roots, "ESTIMATION_CD_TPL_DIR", base_repo_dir / "cd")
    monkeypatch.setattr(template_roots, "ESTIMATION_MD_TPL_DIR", base_repo_dir / "md")
